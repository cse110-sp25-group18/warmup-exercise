...
class GameController {
  ...
  startNewGame() {
    ...
    if (bet <= 0) {
      this.bettingView.showBetStatus(
        "Place a valid bet before starting the game!",
        true,
      );
      return;
    }
  ...
  dealCard(target, faceUp, callback) {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        cardToDeal.controller?.[
          target === "player" ? "enableClick" : "disableClick"
        ]();
  ...
  dealInitialCards() {
    ...
    const dealNext = (index) => {
      ...
      this.dealCard(target, faceUp, () =>
        setTimeout(() => dealNext(index + 1), 700),
      );
    };
